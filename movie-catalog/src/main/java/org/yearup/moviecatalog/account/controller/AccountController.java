...
public class AccountController {
    ...
    public ResponseEntity<?> getAccountById(@PathVariable Long accountId){
        Optional<Account> accounts = accountService.getAccountById(accountId);
        SuccessfulResponseDetail successfulResponseDetail = new SuccessfulResponseDetail(HttpStatus.OK.value(), "Success", accounts);
        return new ResponseEntity(successfulResponseDetail, HttpStatus.OK);
    }

    @RequestMapping(method = RequestMethod.POST, value = "/accounts")
    public ResponseEntity<?> createAccount(@Valid @RequestBody Account accounts){
        Account a = accountService.createAccount(accounts);
